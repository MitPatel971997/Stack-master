...
def main(request):
    ...
    url=url+request.GET.get("question").replace(" ","+")
    ...
    if len(Link)!=0:
	    ...
	    if solution:
		    ...
		    with open("final.html",'w') as question:
		        question.write("{% load static %}")
		        question.write("<html><head><link rel="'stylesheet'" href='{% static 'test.css' %}'></head><body>")
		        qf=request.GET.get
		    ...
		    if len(noSolVote)!=0:
			    ...
			    for k in nodict:
				    if nomax==nodict[k]["vote"]:
					    div1=nodict[k]["ans"]
					    with open(os.path.join("templates","final.html"),'w') as question:
